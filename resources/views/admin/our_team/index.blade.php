    ...
    <div class="row breadcrumbs-top">
     <div class="breadcrumb-wrapper col-12">
      <ol class="breadcrumb">
       ...
       <li class="breadcrumb-item"><a href="{{ route('admin.our_teams.create') }}">Create Our Team</a>
       </li>
       <li class="breadcrumb-item active">Our Team Responsive Datatable
       </li>
      </ol>
     </div>
    </div>
  ...
  <div class="col-12">
   <div class="card">
    <div class="card-header">
     <h4 class="card-title">
      <a href="{{ route('admin.our_teams.create') }}" class="btn btn-success btn-round">New Team Member Create</a>
     </h4>
    ...
    <div class="card-content collapse show">
     <div class="card-body card-dashboard">
      ...
      <table class="table table-striped table-bordered dataex-res-configuration">
       ...
       <tbody>
        @foreach ($our_teams as $key => $team)
        <tr>
         <td>{{ ++$key }}</td>
         <td>{{ $team->name }}</td>
         <td>{{ $team->position }}</td>
         <td>{{ $team->bio }}</td>
         <td><img src="{{ asset('uploads/' . $team->image) }}" width="50px" height="50px"
           alt="{{ $team->name }} Profile">
         </td>
         <td>
          <a href="{{ route('admin.our_teams.edit', $team->id) }}" class="btn btn-warning btn-sm">Edit</a>
          <a href="{{ route('admin.our_teams.show', $team->id) }}" class="btn btn-primary btn-sm">Show</a>
          <form class="d-inline" action="{{ route('admin.our_teams.destroy', $team->id) }}" method="POST">
           @csrf
           @method('DELETE')
